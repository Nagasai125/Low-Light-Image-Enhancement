...
def main():
    ...
    model.enhance.input_conv.apply(model.init_enhancer_weights)
    model.enhance.conv_block.apply(model.init_enhancer_weights)
    model.enhance.output_conv.apply(model.init_enhancer_weights)
    model = model.cuda()
        ...
        if epoch % 50 == 0 and total_step != 0:
            ...
            with torch.no_grad():
                for idx, (input, img_name) in enumerate(test_queue):
                    ...
                    pred1, pred2, denoised, illum_map, illum_down1, illum_down2, enhanced, enhanced_down1, enhanced_down2, content3, illum3, content4, illum4, final_content, final_illum, pred3, pred4, texture_diff1, texture_diff2, blurred_enhanced, blurred_final = model(input)
                    input_name = '%s' % (image_name)
                    final_output = save_images(final_content)
                    enhanced_output = save_images(enhanced)
                    os.makedirs(args.save + '/result/denoise/', exist_ok=True)
                    os.makedirs(args.save + '/result/enhance/', exist_ok=True)
                    Image.fromarray(final_output).save(args.save + '/result/denoise/' + input_name+'_denoise_'+str(epoch)+'.png', 'PNG')
                    Image.fromarray(enhanced_output).save(args.save + '/result/enhance/' +input_name+'_enhance_'+str(epoch)+'.png', 'PNG')
